...
1. **Provide your recommended answer** with every question, prefixed with `💡 My recommendation:`.
   Base the recommendation on the user's context so far and engineering best practices.
1. **Resolve dependencies first.** Walk the decision tree in order — don't ask about edge cases
   before you know the core scope; don't ask about success metrics before you know the goal.
1. **Be relentless.** Keep going until every section of the PRD has enough information.
   Do not stop early just because the user gave a broad answer — follow up to get specifics.
1. **Acknowledge and build.** Start each question with a one-sentence acknowledgement of
   the previous answer before asking the next question.
1. **Probe vague answers.** If an answer is too broad (e.g. "improve performance"), ask a
