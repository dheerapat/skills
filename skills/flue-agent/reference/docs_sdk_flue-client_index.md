...
title: FlueClient | Flue
image: https://flueframework.com/docs/og4.jpg
---

# FlueClient

Last updated Jul 21, 2026[View as Markdown](https://flueframework.com/docs/sdk/flue-client/index.md)
...
Client for one agent conversation, created by [createFlueClient(...)](https://flueframework.com/docs/sdk/create-flue-client/). Every HTTP method rejects with [FlueApiError](https://flueframework.com/docs/sdk/errors/#flueapierror) on a non-2xx response; the stream-backed methods (`wait()`, `observe()`) additionally surface [stream errors](https://flueframework.com/docs/sdk/errors/#stream-errors).

| Field | Description                                                                          |
| ----- | ------------------------------------------------------------------------------------ |
| url   | The fully resolved conversation URL this client addresses, without a trailing slash. |

For chat UIs, `useFlueAgent({ url })` from `@flue/react` wraps this client with maintained React state; see [React](https://flueframework.com/docs/guide/react/).
...
| data-<name>      | A named, client-facing data part streamed by the agent’s useDataWriter writers (AI SDK convention: data-<name> type, payload on data). The name is the part’s identity within the response — a later write updates the part in place, keeping its position. See [Streaming data to the client](https://flueframework.com/docs/guide/agent-hooks/#streaming-data-to-the-client).                                             |
...
`getSnapshot`/`subscribe` match React’s `useSyncExternalStore` contract, and each published update is a new snapshot object identity. `useFlueAgent` from `@flue/react` builds on this method; see [React](https://flueframework.com/docs/guide/react/).
