...
Last updated Jul 21, 2026[View as Markdown](https://flueframework.com/docs/guide/workflows/index.md)
...
To learn how to deploy an agent, visit the [Deploy](https://flueframework.com/docs/guide/deploy/) guide.
...
The smallest workflow is a single [flue run](https://flueframework.com/docs/cli/run/) invocation. It loads the agent module in the local process, submits one message, prints the final reply to stdout, and exits when the run settles — the exit code reports success or failure.
...
Conversations persist between invocations in the project’s [configured database](https://flueframework.com/docs/guide/database/), so reusing an `--id` continues one conversation across runs.
...
The envelope in `triage.json` carries the outcome, the reply, and the conversation id for later steps to use. The [GitHub Actions](https://flueframework.com/docs/ecosystem/deploy/github-actions/) and [GitLab CI](https://flueframework.com/docs/ecosystem/deploy/gitlab-ci/) ecosystem pages cover the platform details.

See the [flue run reference](https://flueframework.com/docs/cli/run/) for agent selection, creation data, and the full flag list.
...
A failed or aborted run rejects the `read()` with `AgentRunError`, so ordinary `try`/`catch` is all the error handling a script needs. The `db` option decides whether conversations outlive the script: omit it for in-memory state that vanishes with the process, or configure a [database adapter](https://flueframework.com/docs/guide/database/) so a later run can continue the same conversation.

See the [init() reference](https://flueframework.com/docs/reference/agent-api/#init) for the rest of the handle’s surface.
...
Flue already guarantees a durable outcome for every individual send: once a message is admitted, that submission settles through crashes, restarts, and redeploys — the [Durability](https://flueframework.com/docs/guide/durability/) guide covers that contract. What Flue does not guarantee is the script _around_ the sends: a workflow that dies between two dispatches re-runs from its start. That is fine for a quick script, but not for a multi-step orchestration that must finish once started.
...
The class is exported from [src/cloudflare.ts](https://flueframework.com/docs/guide/cloudflare-target/#extending-cloudflarets-entrypoint), with its workflow binding declared in `wrangler.jsonc`. See the [Cloudflare target guide](https://flueframework.com/docs/guide/cloudflare-target/) for the rest of the platform details.
