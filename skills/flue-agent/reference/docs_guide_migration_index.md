...
title: Migration Guide | Flue
image: https://flueframework.com/docs/og4.jpg
---

# Migration Guide

Last updated Jul 23, 2026[View as Markdown](https://flueframework.com/docs/guide/migration/index.md)
...
- `flue build` / `flue dev` CLI commands — replaced by [Vite](https://flueframework.com/docs/guide/deploy/) with the `flue()` plugin; `vite dev` and `vite build` are the only commands.
- The auto-mounted `flue()` router and discovery by directory — replaced by [explicit routing](https://flueframework.com/docs/guide/routing/) in `app.ts`; the ['use agent' scan](https://flueframework.com/docs/guide/building-agents/#use-agent-directive) registers agents.
- `defineAgent(async initializer => config)` with a config bag — the agent **is** the function now: an exported capitalized agent function composing behavior with [Agent Hooks](https://flueframework.com/docs/reference/agent-hooks-api/); `defineAgent` is gone entirely.
- Workflows (`defineWorkflow`, `invoke()`, runs, run events) — removed. Use awaited [init() handles](https://flueframework.com/docs/guide/building-agents/#standalone-scripts), [durable tools](https://flueframework.com/docs/guide/tools/#durable-tools), or your own orchestrator. See [Workflows](https://flueframework.com/docs/guide/workflows/).
- The deployment-wide SDK client (`client.agents.*`, `client.workflows.*`) — replaced by the [Flue Agent SDK’s conversation-scoped client](https://flueframework.com/docs/sdk/create-flue-client/): one client per conversation URL.

This guide maps old code onto new APIs; it does not teach the new APIs. Read [Agents](https://flueframework.com/docs/guide/building-agents/), [Agent Hooks](https://flueframework.com/docs/guide/agent-hooks/), [Routing](https://flueframework.com/docs/guide/routing/), and [Workflows](https://flueframework.com/docs/guide/workflows/) first — the sections below assume you know what the replacements are and only cover what to change.
...
- **Registration comes from the ['use agent' scan](https://flueframework.com/docs/guide/building-agents/#use-agent-directive), not the mount.** A dispatch-only agent stays unmounted and still works; mounting registers nothing.
...
- `model` — now [useModel(model, options?)](https://flueframework.com/docs/reference/agent-hooks-api/#usemodel): **required**, exactly once per render, root render only.
- `instructions` — now the agent function’s return string; [useInstruction()](https://flueframework.com/docs/reference/agent-hooks-api/#useinstruction) appends more.
- `tools` — now [useTool()](https://flueframework.com/docs/reference/agent-hooks-api/#usetool) per tool.
- `skills` — now [useSkill()](https://flueframework.com/docs/reference/agent-hooks-api/#useskill) per skill.
- `subagents` (profiles) — now [useSubagent({ name, description, agent, model?, thinkingLevel? })](https://flueframework.com/docs/reference/agent-hooks-api/#usesubagent); the delegate is an agent function, not a profile. `defineAgentProfile` is removed; `defineSubagent()` defines delegates shared across agents.
- `thinkingLevel`, `compaction` — now `useModel(model, { thinkingLevel, compaction })`.
- `sandbox`, `cwd` — now [useSandbox(factory, { cwd })](https://flueframework.com/docs/reference/agent-hooks-api/#usesandbox): at most once per render; presence may be conditional.
...
- `actions` — removed with Actions. Express reusable operations as tools ([harness: true](https://flueframework.com/docs/guide/tools/) for app-driven model work).
...
- If the beta agent parsed setup facts out of its conversation `id` (order numbers, channel refs), move them to creation data: declare a schema with the `initialData` static, pass `initialData` at dispatch, and read it with [useInitialData()](https://flueframework.com/docs/reference/agent-hooks-api/#useinitialdata).

New capabilities you will likely reach for while migrating — durable per-instance state ([usePersistentState](https://flueframework.com/docs/reference/agent-hooks-api/#usepersistentstate)), creation data ([useInitialData](https://flueframework.com/docs/reference/agent-hooks-api/#useinitialdata) + the `initialData` schema static), the delivered-message cursor ([useDelivery](https://flueframework.com/docs/reference/agent-hooks-api/#usedelivery)), self-dispatch ([useDispatchMessage](https://flueframework.com/docs/reference/agent-hooks-api/#usedispatchmessage)), client-facing data parts ([useDataWriter](https://flueframework.com/docs/reference/agent-hooks-api/#usedatawriter)), lifecycle seams ([useAgentStart](https://flueframework.com/docs/reference/agent-hooks-api/#useagentstart)/[useAgentFinish](https://flueframework.com/docs/reference/agent-hooks-api/#useagentfinish)), and response metadata ([useResponseStart/useResponseFinish](https://flueframework.com/docs/reference/agent-hooks-api/#useresponsestart)).
...
[MCP servers](https://flueframework.com/docs/guide/mcp/) are new in this release — `useMcpConnection(...)` mounts a remote server’s tools; nothing migrates.

`harness.fs` is also gone: the harness exposes [harness.sandbox](https://flueframework.com/docs/reference/agent-api/#harnesssandbox), a `SessionEnv` carrying `exec`, the file verbs, `cwd`, and `resolvePath`. Adapters may not support every verb and may expose native accessors (for example Cloudflare Shell’s `shellWorkspace(harness.sandbox)`).
...
See [Sandboxes](https://flueframework.com/docs/guide/sandboxes/).
...
The retry policy that lived on the workflow moves to the agent’s `durability` static. Scheduled workflows follow the same move: a cron trigger now dispatches a signal message to an agent (`dispatch(Agent, { id, message: { kind: 'signal', ... } })`) instead of calling `invoke()` — see [Schedules](https://flueframework.com/docs/guide/schedules/).

In standalone Node scripts (cron jobs, CI, tests), boot the runtime first with `start()` from `@flue/runtime/node`, passing agent functions (or `{ agent, name? }` entries); then `init()`/`dispatch()` work as they do in a server. See [Standalone scripts](https://flueframework.com/docs/guide/building-agents/#standalone-scripts). All of these replacement patterns are covered in depth in the [Workflows](https://flueframework.com/docs/guide/workflows/) guide.
...
**The `dispatchId` → `submissionId` rename.** Pre-release builds used `dispatchId` for dispatch receipts and event correlation; the final vocabulary is one name. `DispatchReceipt.dispatchId` is now `submissionId`, `FlueEvent` carries no `dispatchId` field — `submissionId` alone identifies a submission’s activity, dispatched or direct (see the [Events Reference](https://flueframework.com/docs/reference/events/#flueevent)) — and the telemetry adapters emit `flue.submission.id` instead of `flue.dispatch.id`. New submission ids are `sub_`-prefixed; ids are opaque — do not parse the prefix.
...
Hand-written channels build on the new `createChannelRouter(routes)` from `@flue/runtime` — see [Channels](https://flueframework.com/docs/guide/channels/).

## Database

- Ecosystem adapters now take your driver instead of a connection string: `postgres(process.env.DATABASE_URL!)` becomes `postgres({ query, transaction, close })` wrapping your own `pg` pool. The same runner pattern applies to libsql, mysql, mongodb, and redis; each [ecosystem database page](https://flueframework.com/docs/guide/database/) shows the wrapper.
...
- `registerProvider('ollama', { api, baseUrl, ... })` — now `setProvider(createProvider({ id: 'ollama', auth, models, api }))` with Pi’s `createProvider`. Models are declared as full `Model` objects (each carries its own `baseUrl` and metadata); there is no catalog hydration or zero-fill for custom providers. The [Ollama recipe](https://flueframework.com/docs/guide/models/#custom-providers) is the template.
- `registerProvider('anthropic', { baseUrl, apiKey })` (patch a built-in) — now register your own provider under the built-in’s ID, reusing its catalog models: `models: anthropicProvider().getModels().map((m) => ({ ...m, baseUrl }))`. The [gateway recipe](https://flueframework.com/docs/guide/models/#custom-providers) shows the full shape.
...
See the [Events Reference](https://flueframework.com/docs/reference/events/) for the full envelope (`v: 3`) and payload contract.
...
1. **Build.** Author `vite.config.ts` (`flue()` before `cloudflare()`); move package scripts to `vite dev`/`vite build`; fix `flue.config.ts` (`@flue/runtime/config`, no `root`/`output`); gitignore the generated files.
1. **Routing.** Author explicit mounts in `app.ts`; delete `flue()` router usage; mount each channel’s `route()`; decide which agents are dispatch-only.
1. **Agents.** Convert each initializer to an exported capitalized agent function in a `'use agent'` module: hooks for behavior, statics (`agentName`, `initialData`, `durability`) for the contract, `AgentProps` for the id, platform env instead of `ctx.env`. Convert profiles to `useSubagent` agent functions. Agents that used the implicit virtual sandbox declare one: `useSandbox(bash(() => new Bash({ fs: new InMemoryFs() })))`.
1. **Tools.** Rename `run({ input })` to `run({ data })`; adopt `harness: true` where tools prompted sessions; consider `durable: true` for side-effect sequences.
1. **Skills.** Delete import attributes; let `SKILL.md` imports package themselves; wrap other markdown with `defineSkill` where needed.
1. **Workflows.** Replace each with the smallest fit: awaited `init()` handle, durable tool, or an application-owned orchestrator.
1. **Channels and database.** Rename `conversationKey`/`parseConversationKey` to `instanceId`/`parseInstanceId`; rewrite database adapters around your own driver; move `db.ts` to the source root.
1. **Providers.** Replace `registerProvider()`/`registerApiProvider()` calls with Pi’s `createProvider()` + `setProvider()` (add `@earendil-works/pi-ai` to your dependencies); replace the Cloudflare binding registration with `cloudflareBindingProvider()` from `@flue/runtime/cloudflare/workers-ai`; optionally narrow the shipped providers with `flue({ providers: [...] })` (name `'cloudflare'` to keep Workers AI).
1. **Observability.** Migrate `run_*` handling to `agent_start`/`agent_end`/`submission_settled` and the `instanceId`/`submissionId` correlation fields.
1. **Clients.** Move SDK and React usage to conversation-scoped clients and `useFlueAgent({ url | client })`.
1. **Deployment.** Append `deleted_classes` for `FlueRegistry` and workflow classes; add `new_sqlite_classes` for new agents; plan the drained deployment for the schema reset.
1. **Verify.** Typecheck, tests, a production `vite build`, and a check of the built artifact (exports, merged wrangler config) before deploying.

## Docs Navigation

Current page: [Migration Guide](https://flueframework.com/docs/guide/migration/)
