...
Last updated Jul 21, 2026[View as Markdown](https://flueframework.com/docs/ecosystem/deploy/gitlab-ci/index.md)
...
1. Add a project webhook (**Settings > Webhooks**) that fires on **Issue events**, pointing at a small relay that calls the trigger API with the right variables:
...
An agent’s deterministic orchestration lives in [harness tools](https://flueframework.com/docs/guide/tools/#harness-tools) — finite, schema-validated jobs the model calls — and [skills](https://flueframework.com/docs/guide/skills/), reusable instruction files. Inside either, the harness gives you two core methods:
...
Progress streams to stderr; only the final reply (or the `--json` envelope) lands on stdout. See [flue run](https://flueframework.com/docs/cli/run/) for the full contract.
