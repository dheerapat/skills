...
Last updated Jul 21, 2026[View as Markdown](https://flueframework.com/docs/guide/durability/index.md)

Durability is Flue’s contract for accepted work: once an input is admitted, the runtime owes that conversation a durable terminal outcome — through process crashes, restarts, and redeploys. Where that state is stored is the [Database](https://flueframework.com/docs/guide/database/) guide’s topic. This guide covers submissions and the accepted-work contract, what recovery replays after an interruption, durable tools and delegated tasks, persisted state, per-target recovery behavior, and what is deliberately not durable.

## Submissions and the accepted-work contract

Every input that reaches an agent — a direct HTTP prompt, a [dispatch(...)](https://flueframework.com/docs/guide/building-agents/#dispatch) call, an [init() handle](https://flueframework.com/docs/reference/agent-api/#init)’s `dispatch(...)`, a [channel](https://flueframework.com/docs/guide/channels/) delivery, a [scheduled](https://flueframework.com/docs/guide/schedules/) trigger — is admitted as a **submission**: the payload is recorded durably _before_ any model work begins. That admission record is what the `202` response and the dispatch receipt attest to, and it carries an obligation:
...
The outcome is written as a `submission_settled` record in the conversation’s canonical stream, so anything waiting on the work observes it even across its own reconnects: the Flue Agent SDK’s [wait()](https://flueframework.com/docs/sdk/flue-client/#wait) resolves or rejects from that record, and an awaited `init().read(...)` resolves with the settled reply or rejects with the settled error.
...
The static is applied by the platform while the agent function is _not_ running, so it stays in force after a crash — including a crash in the agent’s own render. The timeout is the total wall-clock budget from the first attempt’s start; turn-boundary joins and response continuations do not extend it. Full field reference: [DurabilityConfig](https://flueframework.com/docs/reference/agent-api/#durabilityconfig).
...
Current page: [Durability](https://flueframework.com/docs/guide/durability/)
