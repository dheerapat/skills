...
1. **Scans for agents.** The ['use agent' scan](https://flueframework.com/docs/guide/building-agents/#use-agent-directive) over your source root defines the application’s agent set, and the generated server bootstrap registers every scanned agent.
1. **Transforms agent modules.** The build stamps each agent’s identity (the function name, or its `agentName` static override) as a string literal bound to the function, so a minified production bundle cannot corrupt the durable identity.
