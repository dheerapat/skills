...
Last updated Jul 21, 2026[View as Markdown](https://flueframework.com/docs/guide/node-target/index.md)
...
For a deployment walkthrough, see [Deploy Agents on Node.js](https://flueframework.com/docs/ecosystem/deploy/node/). To run agents on a cron schedule, see [Schedules](https://flueframework.com/docs/guide/schedules/).

## The built server

[Deploy](https://flueframework.com/docs/guide/deploy/) explains how Vite builds your application — the `app.ts` route map plus every scanned ['use agent' module](https://flueframework.com/docs/guide/building-agents/#use-agent-directive) — into a self-starting server entry at `dist/server.mjs` (plus the non-listening `dist/app.mjs` chunk it imports — what `vite preview` serves, and what a custom host can import to embed the application in its own server):
...
See [Database](https://flueframework.com/docs/guide/database/) for `db.ts`, SQLite, Postgres, and custom adapters. See [Durability](https://flueframework.com/docs/guide/durability/) for recovery behavior.
...
When agent work needs per-session isolation, a Linux toolchain, or a provider-managed environment, use a remote sandbox adapter instead of `local()`. Remote sandboxes run on external infrastructure and connect through the [Sandbox Adapter API](https://flueframework.com/docs/reference/sandbox-api/).
...
- [flue run](https://flueframework.com/docs/cli/run/) loads the project-root `.env`; use `--env <path>` to select one alternate file.
...
Current page: [Node.js](https://flueframework.com/docs/guide/node-target/)
