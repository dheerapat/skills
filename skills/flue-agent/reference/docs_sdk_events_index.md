...
Last updated Jul 21, 2026[View as Markdown](https://flueframework.com/docs/sdk/events/index.md)

The Flue Agent SDK (`@flue/sdk`) delivers conversation data in two vocabularies, at two levels of abstraction:

- **Materialized conversation state** — [FlueConversationState](https://flueframework.com/docs/sdk/flue-client/#flueconversationstate) and [FlueConversationSnapshot](https://flueframework.com/docs/sdk/flue-client/#flueconversationsnapshot): complete, renderable conversations made of messages, parts, and settlements. This is what [observe()](https://flueframework.com/docs/sdk/flue-client/#observe) maintains and [history()](https://flueframework.com/docs/sdk/flue-client/#history) returns, and it is the level application code should consume.
- **Conversation stream chunks** — [ConversationStreamChunk](https://flueframework.com/docs/reference/streaming-protocol/#conversationstreamchunk): the incremental update protocol that `observe()` reduces into state and that [wait()](https://flueframework.com/docs/sdk/flue-client/#wait)’s `onEvent` callback exposes raw.

The runtime’s low-level activity event union ([FlueEvent](https://flueframework.com/docs/reference/events/)) is not part of the SDK: no `FlueClient` method delivers it. In-process consumers subscribe to it with `observe()` from `@flue/runtime`.
...
- `FlueConversationSnapshot`, `FlueConversationState`, `FlueConversationMessage`, `FlueConversationPart`, `FlueConversationSettlement` — the materialized conversation, documented field-by-field on [FlueClient](https://flueframework.com/docs/sdk/flue-client/#flueconversationsnapshot) with `history()` and `observe()`.
...
- `observe()` dedupes by chunk `position` and rehydrates a fresh snapshot on reconnect rather than resuming incrementally; see [observe()](https://flueframework.com/docs/sdk/flue-client/#observe).
...
Current page: [Events](https://flueframework.com/docs/sdk/events/)
