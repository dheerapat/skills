...
Agents are addressable, so you can message with a specific agent by passing an agent ID (`--id`). Agent conversations are persistent — the second message remembers the first. See [flue run](https://flueframework.com/docs/cli/run/) for the full set of flags.
