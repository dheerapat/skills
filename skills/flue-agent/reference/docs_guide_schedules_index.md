...
1. **A delivery.** The trigger’s callback calls `dispatch(agent, { id, message })`. Dispatch resolves when the message is durably admitted to the conversation’s queue — it does not wait for the model to run. Because dispatch addresses the registered agent function directly, a scheduled agent needs no HTTP mount at all (see [Dispatch-only agents](https://flueframework.com/docs/guide/routing/#dispatch-only-agents)).
1. **A conversation.** The `id` you pass names the conversation that receives every fire. Whether that is one continuing conversation or a fresh one per run is a design choice covered [below](#choosing-the-conversation-id).
...
When the scheduled work involves application-controlled steps — reading a data source, writing a report, calling an external API — put those steps behind a [harness tool](https://flueframework.com/docs/guide/tools/#harness-tools) so they behave the same way on every fire.
...
Hooks and tools read the same delivery in code with [useDelivery()](https://flueframework.com/docs/reference/agent-hooks-api/#usedelivery), so a tool can consume `attributes` values directly instead of relying on the model to echo them. The full message shape, including the `tagName` override, is documented at [DeliveredMessage](https://flueframework.com/docs/reference/agent-api/#deliveredmessage).
...
| Id choice                       | Behavior                                                                                                                                                                                        |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fixed ('daily-summary')         | Every fire continues one conversation. The agent sees its previous runs and keeps [persistent state](https://flueframework.com/docs/guide/agent-hooks/#persisted-state) across them.            |
| Per fire (\`daily-${isoDate}\`) | Each fire creates a fresh conversation with bounded context. Pair with initialData to seed the new instance; see [dispatch(...)](https://flueframework.com/docs/reference/agent-api/#dispatch). |
...
The `read()` promise is not itself durable: if the process dies mid-await, the run still settles, but anything after the `await` is gone — so side effects that must not be lost belong inside the agent (a tool call), not after the read. For orchestration that must survive crashes, see [Workflows](https://flueframework.com/docs/guide/workflows/#durable-workflows).

For schedules that live outside a Flue application entirely — a standalone cron script on another machine — boot the runtime with [start()](https://flueframework.com/docs/guide/building-agents/#standalone-scripts) and use the same handle.

## External schedulers

When the platform provides cron as a service — Fly scheduled Machines, Render cron jobs, Railway cron schedules — trigger the deployed application over HTTP instead of running a second process. The scheduler `POST`s a signal to the mounted [conversation URL](https://flueframework.com/docs/guide/routing/#sending-a-message):
...
The server responds `202` at admission, exactly like `dispatch(...)`. This path requires the agent to be mounted, and a mounted agent has no built-in authentication — put the mount behind middleware that verifies the scheduler’s credential, as described in [Protecting your agents](https://flueframework.com/docs/guide/routing/#protecting-your-agents). The [Fly](https://flueframework.com/docs/ecosystem/deploy/fly/), [Render](https://flueframework.com/docs/ecosystem/deploy/render/), and [Railway](https://flueframework.com/docs/ecosystem/deploy/railway/) deploy pages cover each platform’s scheduler and its limits.

## One-shot runs from CI

A scheduler that can run a command — cron itself, a CI pipeline, GitHub Actions — can drive the same agent with [flue run](https://flueframework.com/docs/cli/run/) instead of a live server:
