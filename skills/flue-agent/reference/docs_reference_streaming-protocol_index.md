...
Delivers one message into the conversation. The body is JSON (`Content-Type: application/json`) and is the same `DeliveredMessage` shape a server-side [dispatch(...)](https://flueframework.com/docs/guide/building-agents/#dispatch) admits, with two optional reserved top-level siblings. `@flue/sdk` exports `DeliveredMessage` and `DeliveredAttachment`.
...
Current page: [Streaming Protocol](https://flueframework.com/docs/reference/streaming-protocol/)
