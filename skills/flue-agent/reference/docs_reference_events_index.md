...
1. `submission_running` when the attempt starts processing
1. `operation_start`, `agent_start`
1. `message_start` / `message_end` for the user message
1. `turn_start`, `turn_request`
1. `message_start` for the assistant message; `text_delta`, `thinking_*`, and `toolcall_delta` interleave while it streams
1. `turn`, then `message_end` for the completed assistant message
1. per tool call: `tool_start` when execution begins, then `message_start` / `message_end` for its tool-result message when it finishes
1. the terminal `tool` events when the batch commits, then `turn_messages`
1. further turns repeat from step 5 until a turn produces no tool calls
1. `agent_end`, `operation`, `idle`
1. `submission_settled` when the submission settles
