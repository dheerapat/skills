...
title: Agent Hooks API | Flue
image: https://flueframework.com/docs/og4.jpg
---

# Agent Hooks API

Last updated Jul 23, 2026[View as Markdown](https://flueframework.com/docs/reference/agent-hooks-api/index.md)

This page documents every Flue Hook — the functions an [agent function](https://flueframework.com/docs/reference/agent-api/#agent-functions) calls while it renders — plus the render contract that governs them. All symbols are exported from `@flue/runtime` unless noted. Each hook’s section covers the hook’s own contract — arguments, render rules, when it runs — and defers to the [Agent API](https://flueframework.com/docs/reference/agent-api/) for the resource shapes hooks consume ([defineTool](https://flueframework.com/docs/reference/agent-api/#definetool), [defineSkill](https://flueframework.com/docs/reference/agent-api/#defineskill), [defineSubagent](https://flueframework.com/docs/reference/agent-api/#definesubagent), [defineMcpConnection](https://flueframework.com/docs/reference/agent-api/#definemcpconnection)) and for the programmatic surface _around_ agents (`dispatch()`, `init()`, `start()`, routing, the harness).
...
- `useAgentStart`/`useAgentFinish` are awaited, may be async, and receive a [harness](https://flueframework.com/docs/reference/agent-api/#harness); `useResponseStart`/`useResponseFinish` are synchronous observers — a returned promise fails the submission.
...
- `model` — a model specifier string, `'provider-id/model-id'` (e.g. `'anthropic/claude-sonnet-4-6'`). An unresolvable specifier fails the submission at initialization. See [Models](https://flueframework.com/docs/guide/models/#model-specifier) for the catalog and [Provider API](https://flueframework.com/docs/reference/provider-api/) for registering providers.
...
- `options.compaction` — threshold-compaction configuration ([CompactionConfig](#compactionconfig), below), or `false` to disable threshold compaction. Overflow recovery and explicit [harness.compact()](https://flueframework.com/docs/reference/agent-api/#harnesscompact) still compact when needed.
- Unknown option fields throw.
- Values are submission-scoped: the runtime reads them when a submission starts, so a value computed from state takes effect on the next submission, not mid-run. See [Changing models mid-conversation](https://flueframework.com/docs/guide/models/#changing-models-mid-conversation).
...
- A condition derived from persistent state replays durably: every later submission re-attaches the same declaration, and adapters keyed on the instance id resolve back to the same durable workspace. See [Conditional attachment](https://flueframework.com/docs/guide/sandboxes/#conditional-attachment).
...
Mount a model-callable tool for the current render. Accepts a [defineTool(...)](https://flueframework.com/docs/reference/agent-api/#definetool) value or an inline definition object — the same validation applies at the mount site. Whether called in the agent body or a custom hook, the tool joins the render’s single flat tool set. What a tool _is_ — the full definition contract, `ToolContext`, and the `harness`/`durable` flags — is documented at [defineTool()](https://flueframework.com/docs/reference/agent-api/#definetool) in the Agent API.

- Mounts may be conditional; set changes are narrated to the model (see [Dynamic resources](https://flueframework.com/docs/reference/agent-api/#dynamic-resources)). An unmounted tool cannot be called at all.
...
- Accepts a `SkillReference` — the value of a `SKILL.md` import (packaged automatically by the build; see [Skills](https://flueframework.com/docs/guide/skills/#import-and-mount-a-skill)) or a [defineSkill(...)](https://flueframework.com/docs/reference/agent-api/#defineskill) result — or an inline [SkillDefinition](https://flueframework.com/docs/reference/agent-api/#skilldefinition) object, validated at the mount site. The definition contract — every field and constraint — is documented in the Agent API.
- Mounting the same skill name twice in one render throws.
- Mounts may be conditional; catalog changes are narrated (see [Dynamic resources](https://flueframework.com/docs/reference/agent-api/#dynamic-resources)).
...
Declare a delegate the model can hand focused work to via the framework’s `task` tool. Delegation is a declared capability: the `task` tool is always in the tool set with a fully static spec (either changing would rewrite the serialized tools block and invalidate the provider’s prompt cache), the roster lives in the system prompt’s “Available Agents” section, and the tool’s required `agent` parameter only resolves against declared subagents — with an empty roster the tool is inert. The definition shape — [SubagentDefinition](https://flueframework.com/docs/reference/agent-api/#subagentdefinition), the [defineSubagent()](https://flueframework.com/docs/reference/agent-api/#definesubagent) helper, and the blank general-purpose [GeneralSubagent](https://flueframework.com/docs/reference/agent-api/#generalsubagent) — is documented in the Agent API.

- Duplicate delegate names in one render throw. Declarations may be conditional; roster changes are narrated ([Dynamic resources](https://flueframework.com/docs/reference/agent-api/#dynamic-resources)).
...
- The delegate is isolated from the parent: nothing flows in except the shared environment and, unless overridden on the definition, the parent’s model and reasoning effort. It runs a detached session, and only its final text returns to the parent. See [Subagents](https://flueframework.com/docs/guide/subagents/) and the [subagent-render hook rules](#rendering-and-the-rules-of-hooks).
...
- Writes made by tools become durable atomically with the tool batch that made them: if the batch settles, the write is durable; if recovery settles the batch as interrupted, the write never happened. See [Durability](https://flueframework.com/docs/guide/durability/#persisted-state).
...
- With an [initialData schema static](https://flueframework.com/docs/reference/agent-api/#agent-statics) on the agent, the value is validated at creation (a mismatch fails the creating send, so the value is always present here) and the hook returns the schema-parsed output. Without a schema, whatever the creator sent is returned untyped.
...
- [Framework narration signals](https://flueframework.com/docs/reference/agent-api/#dynamic-resources) (`resources`/`instructions`/`environment`) do not advance the cursor — bookkeeping about the agent’s own declared surface never displaces the input the response is answering.
...
Get a dispatcher bound to this agent instance — the agent-scoped form of the top-level [dispatch()](https://flueframework.com/docs/reference/agent-api/#dispatch). The returned function takes just the message: the instance already exists, so there is no `initialData` and no `uid` condition. Semantics are identical to the global verb by construction — same queue, same admission, same delivery, one accepted order shared with direct HTTP prompts.
...
- Parts land on the wire as data parts of the conversation message and on [AgentReply.data](https://flueframework.com/docs/reference/agent-api/#agentreply); see [Streaming data to the client](https://flueframework.com/docs/guide/agent-hooks/#streaming-data-to-the-client).
...
- `ctx.harness` is the [harness](https://flueframework.com/docs/reference/agent-api/#harness), materialized lazily on first access. `ctx.signal` is the submission’s abort signal. `ctx.log` emits progress lines into the conversation stream; the model never sees them.
...
Run a callback when the agent would otherwise finish responding — the model has no more tool calls and the response is about to settle. The enforcement seam: inspect what the response actually did and, if the work is not done, `append` a signal to send the model back to work within the same response. Callbacks are awaited and may be async, receive a [harness](https://flueframework.com/docs/reference/agent-api/#harness), and a throw fails the submission — the [shared event-hook contract](#rendering-and-the-rules-of-hooks).
...
- Runaway protection is a fixed framework ceiling of 32 continuation cycles per response, not configurable: a hook that appends unconditionally fails the submission loudly instead of settling as a success. The submission’s [durability timeout](https://flueframework.com/docs/reference/agent-api/#durabilityconfig) remains the total wall-clock backstop — neither continuations nor joins extend it.
...
- Metadata is model-invisible and non-reactive; the runtime stamps no keys of its own. It reaches clients on the conversation stream and on [AgentReply.metadata](https://flueframework.com/docs/reference/agent-api/#agentreply).
...
- [Agent API](https://flueframework.com/docs/reference/agent-api/) — the agent module contract, `dispatch()`, `init()`, `start()`, routing, the harness surface, and the `defineTool`/`defineSkill`/`defineSubagent` resource helpers.
- [Agent Hooks guide](https://flueframework.com/docs/guide/agent-hooks/) — the walkthrough of hooks, state, event hooks, and data writers.
- [Tools](https://flueframework.com/docs/guide/tools/), [Skills](https://flueframework.com/docs/guide/skills/), [Subagents](https://flueframework.com/docs/guide/subagents/), [Models](https://flueframework.com/docs/guide/models/), [Sandboxes](https://flueframework.com/docs/guide/sandboxes/) — per-capability guides.
- [Errors](https://flueframework.com/docs/reference/errors/) — the error classes hooks and tools throw.

## Docs Navigation

Current page: [Agent Hooks API](https://flueframework.com/docs/reference/agent-hooks-api/)
