...
1. Where should it deploy? For example: Cloudflare Workers, Node.js, GitHub Actions, GitLab CI/CD, Vercel, Fly.io.
   ...
   - Deploy on GitHub Actions: https://flueframework.com/docs/ecosystem/deploy/github-actions/index.md
...
1. Create or update the project in the requested directory using the selected source layout.
1. Always create one minimal **agent module** matching the user's idea, keeping it closer to "hello world" than a production app.
...
1. If the selected shape is **agent + workflow**, create one minimal **workflow module** for the finite job.
...
1. Add `tsconfig.json` for TypeScript editor/typechecking support.
...
1. Add only the dependencies and config required by the selected deploy guide and chosen starter shape.
1. Run the most relevant validation command you can, such as build, typecheck, or `flue run` when a finite agent prompt or workflow was created. If you cannot run it, explain why.
1. Finish with the exact next commands the user should run, including how to set any required secrets and how to interact with the agent definition or invoke the workflow.
